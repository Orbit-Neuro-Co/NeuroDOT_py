...
for gidx = 1:ng
  ...
  % fail safe for extra metadata
  try
    tt_mpu_dt = data(gidx).node_mpu_dt;
  catch
    tt_mpu_dt = 0; % Assign default value if the field does not exist
  end

  % Construct time vectors
  if strcmp(time_style, 'explicit')
    tt_chn = (0:((data(gidx).nframes)-1))*data(gidx).chn_dt;
    tt_mpu = (0:((size(data(gidx).node_acc, 3))-1))*tt_mpu_dt;
  else
